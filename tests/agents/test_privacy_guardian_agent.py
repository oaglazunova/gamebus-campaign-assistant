...
def test_privacy_guardian_stores_compatibility_policy_and_rich_state(tmp_path: Path):
	...
	assert "privacy_report" in ctx.shared
	assert "access_policy" in response.payload
	assert "privacy_summary" in response.payload
	assert "privacy_report" in response.payload

	privacy = ctx.shared["privacy"]
	privacy_state = ctx.shared["privacy_state"]
	privacy_report = ctx.shared["privacy_report"]

	assert "structural_change_agent" in privacy
	assert "agent_policies" in privacy_state
	assert privacy_state["summary"]["policy_mode"] == "coarse_grained_phase_2_step_11"
	assert privacy_report["policy_mode"] == "coarse_grained_phase_2_step_11"
	assert "policy_sources_by_agent" in privacy_report
	assert "semantic_agents_requiring_views" in privacy_report
	assert "override_warning_count" in privacy_report
