...
TTM_AGENT_NAME = "ttm_grounding_agent"
TTM_SHARED_KEY = "ttm_grounding"
LEGACY_THEORY_SHARED_KEY = "theory_grounding"


class TTMGroundingAgent(BaseAgent):
    """
    TTM-specific grounding agent.

    Release 2 intentionally avoids presenting this as a general theory-grounding
    component. It is a bounded TTM reference/check-summary layer that runs only
    when the campaign profile or attached theory sources indicate TTM.
    """
    ...
    def _store_payload(self, context: AgentContext, payload: dict[str, Any]) -> None:
        # Canonical Release 2 key.
        context.shared[TTM_SHARED_KEY] = payload

        # Temporary compatibility key used by existing UI/helpers during migration.
        context.shared[LEGACY_THEORY_SHARED_KEY] = payload
    ...
    def run(self, context: AgentContext) -> AgentResponse:
        ...
        if not uses_ttm or not ttm_enabled:
            ...
            return AgentResponse(
                agent_name=self.name,
                success=True,
                summary="TTM grounding skipped because TTM is not enabled for this campaign.",
                payload=payload,
                warnings=[],
            )
        ...
        if failed_checks_seen:
            ...
            notes.append("TTM grounding is operating with incomplete TTM resources.")
            confidence = "low"
        else:
        ...
        return AgentResponse(
            agent_name=self.name,
            success=True,
            summary="TTM grounding completed.",
            payload=payload,
            warnings=warnings,
        )
